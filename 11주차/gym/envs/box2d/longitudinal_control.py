...
class LongitudinalController:
    ...
    functions:
        ...
        control()
    '''
    def __init__(self, KP=0.01, KI=0, KD=0):
        self.last_error = 0
        self.sum_error = 0
        self.last_control = 0
        self.speed_history = []
    ...
    def PID_step(self, speed, target_speed):
        ...
        et_0 = target_speed - speed
        if not self.target_speed_history:
            et_1 = 0
        else:
            et_1 = self.target_speed_history[-1] - self.speed_history[-1]

        sum1 = sum(self.target_speed_history)
        sum2 = sum(self.speed_history)
        # derive PID elements
        ut = (self.KP)*(et_0) + (self.KD)*(et_0 - et_1) + self.KI*(sum1+target_speed - sum2-speed)
