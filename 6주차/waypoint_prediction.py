...
def waypoint_prediction(roadside1_spline, roadside2_spline, num_waypoints=6, way_type = "smooth"):
    ...
    if way_type == "center":
        ...
        '''
        arr1 = roadside1_points.reshape(1,-1)
        arr2 = roadside2_points.reshape(1,-1)
        way_points_center = (np.array((arr1+arr2)/2)).reshape(-1)
        #print(way_points_center.reshape(-2,1))
        ...
        Example)
            ...
            minimize Parameters Hint)
                ...
                args    = ?
        '''
        temp = (np.array(roadside1_points.reshape(1,-1))).reshape(-1)
        way_points = minimize(fun = smoothing_objective, x0 = temp, args = way_points_center)
        return way_points['x'].reshape(2,-1)
...
def target_speed_prediction(waypoints, num_waypoints_used=5,
    ...
    tempexp = num_waypoints_used - 2 - curv_center
    target_speed = (max_speed - min_speed)*np.exp(-exp_constant*abs(tempexp)) + min_speed
    #print(target_speed)
    return target_speed